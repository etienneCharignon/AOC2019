...
const {
  ...
  closest_disctances_intersections
} = require('../fuel_management_system.js');

describe.only('day3: fuel management system', function() {
  it('compute all positions to the right', function() {
    expect(collectPoints('R1')).to.eql([{x:1,y:0}]);
    expect(collectPoints('R2')).to.eql([{x:1,y:0}, {x:2,y:0}]);
  });
  it('compute all positions to the left', function() {
    expect(collectPoints('L1')).to.eql([{x:-1,y:0}]);
  });
  it('compute all positions up', function() {
    expect(collectPoints('U1')).to.eql([{x:0,y:1}]);
  });
  it('compute all positions down', function() {
    expect(collectPoints('D1')).to.eql([{x:0,y:-1}]);
  });

  it('compute a path', function() {
    expect(collectPoints('R2,U1')).to.eql([{x:1,y:0}, {x:2,y:0}, {x:2,y:1}]);
  });

  it('find intersections between two paths', function() {
    expect(intersections([{x:1,y:0}, {x:2,y:0}, {x:2,y:3}], [{x:2,y:0}]))
      .to.eql([{x:2,y:0}]);
  });
...
//       .to.equal(1983);
    expect(closest_disctances_intersections(
      ...
      collectPoints('U62,R66,U55,R34,D71,R55,D58,R83')))
      .to.equal(159);
  });

const expect = require('expect.js');
const basePattern = [ 0, 1, 0, -1 ];
const { elementPattern, computeElement,compute } = require('../day16-flawed-frequency-transmission');

describe.only('day 16 - Flawed Frequency Transmission', function () {
  it('compute the first element pattern', function () {
    const input = "12345678";
    expect(elementPattern(1,input, basePattern)).to.eql([1,0,-1,0,1,0,-1, 0]);
  });
  it('compute the second element pattern', function () {
    const input = "12345678";
    expect(elementPattern(2,input, basePattern)).to.eql([0,1,1,0,0,-1,-1,0]);
  });
  it('comute first element fist generation', function () {
    const input = "12345678".split('').map(c => parseInt(c));
    expect(computeElement(input, elementPattern(1, input, basePattern))).to.eql(4);
  });

  it('compute first generation', function () {
    const input = "12345678".split('').map(c => parseInt(c));
    expect(compute(1, input, basePattern)).to.eql([4,8,2,2,6,1,5,8]);
  });

  it('compute the puzzel input', function () {
    const puzzelInput = ''.split('').map(c => parseInt(c));

    var signal = puzzelInput;
    for(var i = 0; i<100;i++) {
      signal = compute(i, signal, basePattern);
    }
    expect(signal.slice(0,8).join('')).to.eql('');
  });
});
